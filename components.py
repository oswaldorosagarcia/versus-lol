...
def get_rune_html(rune_id):
    ...
    if rune_id in rune_urls: 
        return f"<img src='{rune_urls[rune_id]}' width='14' style='background-color:#000; border-radius:50%; border: 1px solid #333;'>"
    return "<div style='width:14px; height:14px; background-color:#222; border-radius:50%; border: 1px solid #333;'></div>"
...
def build_item_html(items):
    ...
    for item in items:
        if item == 0: 
            html += "<div style='width:18px; height:18px; background-color:rgba(0,0,0,0.5); border:1px solid #333; border-radius:4px;'></div>"
        else: 
            html += f"<img src='https://ddragon.leagueoflegends.com/cdn/15.4.1/img/item/{item}.png' width='18' height='18' style='border:1px solid #555; border-radius:4px;'>"
    return html + "</div>"
...
def build_stat_bar(label, val1, val2, suffix="", invert_colors=False):
    ...
    <div style="margin-bottom: 20px; padding: 0 10px;">
        <div style="display:flex; justify-content:space-between; margin-bottom:5px; align-items:flex-end;">
            <span style="color:{c1}; font-weight:900; font-size:0.9rem;">{s1}{suffix}</span>
            <span style="color:#FFF; font-weight:bold; font-style:italic; font-size:0.6rem; letter-spacing: 1px;">{label}</span>
            <span style="color:{c2}; font-weight:900; font-size:0.9rem;">{s2}{suffix}</span>
        </div>
...
def build_overview_header_html(p_data):
    ...
    for b in dash['badges']:
        ...
        badges_html += f"<span class='profile-badge' title='{desc}' style='background-color: {bg}; border: 1px solid {color}66; padding: 3px 8px; border-radius: 12px; font-size: 0.6rem; color: {color}; font-weight: 900; text-transform: uppercase; box-shadow: 0 4px 10px {bg}; display: inline-flex; align-items: center; gap: 4px; letter-spacing: 0.5px;'><span style='font-size: 0.7rem;'>{icon}</span> {b}</span>"
    ...
    <div class='overview-header' style='margin-bottom: 20px;'>
        <div style='display:flex; align-items:center; gap:25px;'>
            <img src='{icon_url}' width='60' style='border: 3px solid #333; border-radius: 16px; box-shadow: 0 0 10px rgba(0,0,0,0.5);'>
            <div>
                <h1 style='margin:0 0 5px 0; color:#FFF !important; font-size: 1.4rem; letter-spacing: 1px;'>{p_data['player']} <span style='color:#888; font-size: 0.9rem; text-transform:none; font-weight: normal;'>#{p_data.get('tag', '')}</span></h1>
        ...
        <div style='display:flex; align-items:center; gap:20px; border-left: 1px solid #333; padding-left:30px;'>
            ...
            <div style='min-width: 220px;'>
                <p style='margin:0 0 2px 0; color:#888; font-size:0.6rem; font-weight:bold; letter-spacing: 0.5px;'>{p_data.get('rank_type', 'Ranked Solo')}</p>
                <h2 style='margin:0; color:#1E88E5 !important; font-size:1rem; white-space: nowrap;'>{rank_str} <span style='color:#FFF; font-size:0.8rem;'>{current_lp} LP</span></h2>
...
def build_match_card_html(g):
    ...
    st_b = f"<span style='background-color: {'rgba(30,136,229,0.2)' if is_w else 'rgba(211,47,47,0.2)'}; color: {b_c}; padding: 3px 8px; border-radius: 8px; font-weight: 900; font-size: 0.7rem; letter-spacing: 1px;'>{'VITÓRIA' if is_w else 'DERROTA'}</span>"
        ...
        <div style='display:flex; justify-content:space-between; align-items:center; padding: 12px;'>
            <div style='display:flex; align-items:center; gap:12px; width:30%;'>
                <div style='text-align:center; min-width:80px;'>
                    {st_b}
                    <p style='color:#888; font-size:0.6rem; margin-top:10px;'><b>{g.get('queue', 'Ranked')}</b></p>
                    <p style='color:#888; font-size:0.55rem; margin:0;'>{g.get('time_ago', '')}</p>
                    <p style='color:#888; font-size:0.55rem; margin:0;'>{format_time(g.get('duration', 0))}</p>
                </div>
                <div style='position:relative;'>
                    <img src='{get_champ_img(g["champion"])}' width='38' style='border-radius:50%; border:2px solid {b_c};'>
                    <div style='position:absolute; bottom:-3px; right:-3px; background:#000; color:#FFF; font-size:0.5rem; font-weight:bold; border-radius:50%; border:1px solid {b_c}; width:16px; height:16px; display:flex; align-items:center; justify-content:center;'>{g.get('level', 1)}</div>
                </div>
                <div style='display:flex; flex-direction:column; gap:2px;'>
                    <img src='{get_spell_img(g.get("spells", [4,4])[0])}' width='14' style='border-radius:3px;'>
                    <img src='{get_spell_img(g.get("spells", [4,4])[1])}' width='14' style='border-radius:3px;'>
                </div>
...
def build_duel_player_card(player, title, color):
    ...
    <div class='data-card' style='border-top: 8px solid {color}; text-align:center; background:#111;'>
        ...
        <img src='{get_champ_img(player['champion'])}' width='70' style='border-radius: 50%; border: 4px solid {color}; box-shadow: 0 0 15px {color}44;'>
        <h1 style='font-size:1.4rem; margin-top:10px;'>{player['champion']}</h1>
