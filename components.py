...
def get_rune_html(rune_id):
    ...
    if rune_id in rune_urls: 
        return f"<img src='{rune_urls[rune_id]}' width='18' style='background-color:#000; border-radius:50%; border: 1px solid #333;'>"
    return "<div style='width:18px; height:18px; background-color:#222; border-radius:50%; border: 1px solid #333;'></div>"
...
def build_item_html(items):
    ...
    for item in items:
        if item == 0: 
            html += "<div style='width:24px; height:24px; background-color:rgba(0,0,0,0.5); border:1px solid #333; border-radius:6px;'></div>"
        else: 
            html += f"<img src='https://ddragon.leagueoflegends.com/cdn/15.4.1/img/item/{item}.png' width='24' height='24' style='border:1px solid #555; border-radius:6px;'>"
    return html + "</div>"
...
def build_stat_bar(label, val1, val2, suffix="", invert_colors=False):
    ...
    <div style="margin-bottom: 20px; padding: 0 10px;">
        <div style="display:flex; justify-content:space-between; margin-bottom:5px; align-items:flex-end;">
            <span style="color:{c1}; font-weight:900; font-size:1.1rem;">{s1}{suffix}</span>
            <span style="color:#FFF; font-weight:bold; font-style:italic; font-size:0.75rem; letter-spacing: 1px;">{label}</span>
            <span style="color:{c2}; font-weight:900; font-size:1.1rem;">{s2}{suffix}</span>
        </div>
...
def build_overview_header_html(p_data):
    ...
    if p_tier_raw in ["", "UNRANKED", "NONE"]:
        ...
        tier_html = f"<img src='{get_tier_img(p_tier_raw)}' width='60' style='filter: drop-shadow(0 0 8px rgba(0,0,0,0.5));'>"
    ...
    for b in dash['badges']:
        ...
        badges_html += f"<span class='profile-badge' title='{desc}' style='background-color: {bg}; border: 1px solid {color}66; padding: 4px 10px; border-radius: 16px; font-size: 0.7rem; color: {color}; font-weight: 900; text-transform: uppercase; box-shadow: 0 4px 10px {bg}; display: inline-flex; align-items: center; gap: 4px; letter-spacing: 0.5px;'><span style='font-size: 0.85rem;'>{icon}</span> {b}</span>"
    ...
    <div class='overview-header' style='margin-bottom: 20px;'>
        <div style='display:flex; align-items:center; gap:25px;'>
            <img src='{icon_url}' width='80' style='border: 3px solid #333; border-radius: 20px; box-shadow: 0 0 15px rgba(0,0,0,0.5);'>
            <div>
                <h1 style='margin:0 0 5px 0; color:#FFF !important; font-size: 1.8rem; letter-spacing: 1px;'>{p_data['player']} <span style='color:#888; font-size: 1.2rem; text-transform:none; font-weight: normal;'>#{p_data.get('tag', '')}</span></h1>
        ...
        <div style='display:flex; align-items:center; gap:20px; border-left: 1px solid #333; padding-left:30px;'>
            ...
            <div style='min-width: 220px;'>
                <p style='margin:0 0 2px 0; color:#888; font-size:0.75rem; font-weight:bold; letter-spacing: 0.5px;'>{p_data.get('rank_type', 'Ranked Solo')}</p>
                <h2 style='margin:0; color:#1E88E5 !important; font-size:1.3rem; white-space: nowrap;'>{rank_str} <span style='color:#FFF; font-size:0.95rem;'>{current_lp} LP</span></h2>
...
def build_match_card_html(g):
    ...
    team1_html = "".join([f"<div style='display:flex; align-items:center; gap:4px; margin-bottom:2px;'><img src='{get_champ_img(p['champ'])}' width='14' height='14' style='border-radius:4px;'><span style='white-space:nowrap; overflow:hidden; text-overflow:ellipsis; max-width:65px;' title='{p['name']}'>{p['name']}</span></div>" for p in g['team100']])
    team2_html = "".join([f"<div style='display:flex; align-items:center; gap:4px; margin-bottom:2px;'><img src='{get_champ_img(p['champ'])}' width='14' height='14' style='border-radius:4px;'><span style='white-space:nowrap; overflow:hidden; text-overflow:ellipsis; max-width:65px;' title='{p['name']}'>{p['name']}</span></div>" for p in g['team200']])
    
    feedbacks_html = "".join([f"<span style='background-color:rgba(255,255,255,0.05); border:1px solid rgba(255,255,255,0.1); color:#DDD; font-size:0.6rem; padding:3px 8px; border-radius:16px; font-weight:bold; letter-spacing:0.5px;'>{f}</span>" for f in g.get('feedbacks', [])])
        ...
        <div style='display:flex; justify-content:space-between; align-items:center; padding: 12px;'>
            <div style='display:flex; align-items:center; gap:12px; width:30%;'>
                ...
                <div style='position:relative;'>
                    <img src='{get_champ_img(g["champion"])}' width='50' style='border-radius:50%; border:2px solid {b_c};'>
                ...
                <div style='display:flex; flex-direction:column; gap:2px;'>
                    <img src='{get_spell_img(g.get("spells", [4,4])[0])}' width='18' style='border-radius:4px;'>
                    <img src='{get_spell_img(g.get("spells", [4,4])[1])}' width='18' style='border-radius:4px;'>
                </div>
            ...
            <div style='width:12%; text-align:center;'>
                <p style='color:#FFF; margin:0; font-size:0.95rem; font-weight:bold;'>{g.get('kills',0)} / <span style='color:#D32F2F;'>{g.get('deaths',0)}</span> / {g.get('assists',0)}</p>
                <p style='color:{kda_color}; margin:0; font-size:0.75rem; font-weight:bold;'>{kda_ratio}:1 KDA</p>
            </div>
            <div style='width:12%; text-align:center;'>
                <p style='margin:0; color:#FFF; font-weight:bold; font-size:0.85rem;'>{g.get('cs',0)} CS</p>
                <p style='margin:0; color:#888; font-size:0.75rem;'>({csm}/m)</p>
                <p style='margin:2px 0 0 0; color:#FF2A2A; font-weight:bold; font-size:0.75rem;'>{g.get('kp', 0)}% KP</p>
            </div>
            ...
            <div style='width:12%; display:flex; flex-direction:column; align-items:center; justify-content:center; border-left: 1px solid #333;'>
                ...
                <img src='{get_champ_img(g.get("rival_champ", "Unknown"))}' width='45' style='border-radius:50%; border:2px solid {r_b_c};'>
...
def build_duel_player_card(player, title, color):
    ...
    <div class='data-card' style='border-top: 8px solid {color}; text-align:center; background:#111;'>
        ...
        <img src='{get_champ_img(player['champion'])}' width='90' style='border-radius: 50%; border: 4px solid {color}; box-shadow: 0 0 20px {color}44;'>
        <h1 style='font-size:1.8rem; margin-top:10px;'>{player['champion']}</h1>
